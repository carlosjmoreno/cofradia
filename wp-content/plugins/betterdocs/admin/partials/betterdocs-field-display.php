...
<tr data-id="<?php echo $key; ?>" id="<?php echo esc_attr( $id ); ?>" class="betterdocs-field <?php echo $row_class.' type-'.$field['type']; ?>">
    ...
    <?php elseif( $field['type'] == 'card' && !empty( $field['label'] )) : ?>
        <td class="betterdocs-control betterdocs-card"><a href="<?php echo esc_url($field['url']) ?>"><div class="betterdocs-card-content"><img src="<?php echo BETTERDOCS_ADMIN_URL; ?>/assets/img/betterdocs-customize.svg" alt="betterdocs-documentation"><p><?php _e('Design your templates and pages with live customizer.', 'betterdocs'); ?></p><p class="betterdocs-customize-button"><?php esc_html_e( $field['label'], 'betterdocs' ); ?></p></div></a></td>
    <?php else: ?>
