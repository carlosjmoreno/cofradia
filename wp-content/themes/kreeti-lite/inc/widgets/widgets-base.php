 ...
 * Base Widget Class
 */
class Kreeti_Widget_Base extends WP_Widget
{
    /**
     * @var Array of string
     */
     ...
     */
    public function kreeti_generate_section_title($field, $label, $note = '', $class = '', $css='')
    {
     ...
     * @param string $note field note to appear below
     */
    public function kreeti_generate_text_input($field, $label, $value, $type = 'text', $note = '', $class = '')
    {
     ...
     * @param string $note field note to appear below
     */
    public function kreeti_generate_textarea($field, $label, $note = '')
    {
     ...
     */
    public function kreeti_generate_checkbox_input($field, $label, $elements, $note = '')
    {
     ...
     * @param Array_A $elements
     */
    public function kreeti_generate_select_options($field, $label, $elements, $note = '')
    {
    ...
    public function kreeti_generate_image_upload($field, $label, $value, $note = '', $class = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : '';
        ?>
        <div>
            ...
            <div class="image-preview-wrap">
                ...
                <input type="button" class="select-img button button-primary"
                       value="<?php esc_attr_e('Upload', 'kreeti-lite'); ?>"
                       data-uploader_title="<?php esc_attr_e('Select Image', 'kreeti-lite'); ?>"
                       data-uploader_button_text="<?php esc_attr_e('Choose Image', 'kreeti-lite'); ?>"/>
                <?php
                    $image_status = false;
                    if (!empty($instance)) {
                        $image_status = true;
                ...
                ?>
                <input type="button" value="<?php echo _x('X', 'Remove', 'kreeti-lite'); ?>"
    ...
    {
        $instance = array();
        $instance = $this->kreeti_sanitize_data($instance, $new_instance);
        return $instance;
    }

    public function kreeti_sanitize_data($instance, $new_instance)
    {
        if (is_array($this->text_fields)) {
            // update the text fields values
            foreach ($this->text_fields as $field) {
                $instance = array_merge($instance, $this->kreeti_update_text($field, $new_instance));
            }
        }
        ...
        if (is_array($this->url_fields)) {
            ...
            foreach ($this->url_fields as $field) {
                $instance = array_merge($instance, $this->kreeti_update_url($field, $new_instance));
            }
        }
        ...
        if (is_array($this->text_areas)) {
            ...
            foreach ($this->text_areas as $field) {
                $instance = array_merge($instance, $this->kreeti_update_textarea($field, $new_instance));
            }
        }
        if (is_array($this->checkboxes)) {
            // update the checkbox fields values
            foreach ($this->checkboxes as $field) {
                $instance = array_merge($instance, $this->kreeti_update_checkbox($field, $new_instance));
            }
        }
        if (is_array($this->select_fields)) {
            // update the select fields values
            foreach ($this->select_fields as $field) {
                $instance = array_merge($instance, $this->kreeti_update_select($field, $new_instance));
            }
        }
        return $instance;
    }
     ...
     */
    public function kreeti_update_text($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function kreeti_update_url($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function kreeti_update_textarea($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function kreeti_update_checkbox($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function kreeti_update_select($name, $new_instance)
    {
        $instance = array();
